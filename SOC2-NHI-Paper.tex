...
\IEEEPARstart{W}{alk} through the IAM console of any mid-sized cloud deployment and two populations become visible---though security tooling tends to acknowledge only one. The human-account population is well-trodden ground: decades of practitioner experience have produced a dense ecosystem of controls for password hygiene, second-factor enrollment, quarterly access recertification, and trigger-based deprovisioning. The second population receives no comparable attention. Service principals, API tokens, deploy keys, and workflow secrets operate continuously in the background, authenticating thousands of inter-service calls per hour, accumulating permissions over time, and aging past any sensible rotation deadline---largely unobserved.

These non-human identities---the machine credentials stitching together modern microservice architectures---have grown far beyond what most security programmes were designed to govern. Gartner's 2024 IAM predictions estimated that 75\% of cloud security failures through 2025 would trace back to inadequate management of machine identities~\cite[p.\,4]{gartner2024nhi}. The 2024 Verizon Data Breach Investigations Report found credential compromise behind 31\% of all breaches, with service account abuse representing a growing share~\cite[p.\,8]{verizon2024dbir}. The OWASP Non-Human Identities Top~10---released in its inaugural 2025 edition---catalogues ten distinct attack categories from improper offboarding through long-lived secrets to human misuse of machine credentials~\cite{owaspnhi2025}.
...
Against this backdrop, the work reported here pursues five goals:
\begin{enumerate}
...
The term ``non-human identity'' encompasses any programmatic credential that software, services, or automation use for authentication without real-time human involvement~\cite{hashicorp2023secrets}. Unlike human accounts---which have dedicated lifecycle workflows for onboarding, recertification, and termination---NHIs are typically provisioned for narrow operational purposes (a CI/CD pipeline credential, a monitoring agent token, a microservice authentication secret) and then forgotten. They accumulate without inventory. They age without rotation. And they retain over-privileged scopes inherited from initial provisioning decisions made months or years earlier.

The inaugural OWASP Non-Human Identities Top~10, released in 2025, provides a structured taxonomy of machine-credential failure modes~\cite{owaspnhi2025}. Four categories bear directly on this work. \emph{Improper offboarding} (NHI1) concerns credentials persisting after the service or personnel they were provisioned for have been retired. \emph{Secret leakage} (NHI2) captures incidents where credentials are inadvertently embedded in source code or execution logs. \emph{Overprivilege} (NHI5) describes the common condition where a credential's granted scope substantially exceeds its operational requirement. \emph{Long-lived secrets} (NHI7) characterises credentials lacking defined expiry dates or rotation schedules. Each of these failure modes can be present in a production environment that holds a current, unqualified SOC~2 opinion.

\subsection{Related Work}

\textbf{Detection-focused tools.} GitGuardian~\cite{gitguardian2024} detects hardcoded secrets in source repositories---a valuable but narrow capability addressing NHI2 (Secret Leakage) alone, without assessing credential age, privilege scope, or compliance posture. Its 2024 report notes that 70\% of secrets first exposed in 2022 remained valid two years later~\cite[p.\,12]{gitguardian2024}, underscoring the staleness dimension our scoring model captures. Microsoft Entra Workload Identities~\cite{entra2024} provides governance for Azure service principals and managed identities but ignores GitHub PATs, deploy keys, and TLS certificates entirely. CertSpotter~\cite{sslmate2024} monitors TLS certificate issuance via Certificate Transparency logs but lacks integration with enterprise credential rotation workflows. None of these tools produces a compliance posture metric or maps findings to audit controls.
...
A survey of published tools and standards reveals no framework that simultaneously addresses version control credentials, cloud identity provider secrets, and PKI certificates within a unified governance model---let alone one with explicit SOC~2 control mappings and continuously updated compliance measurement. The gap reflects a broader pattern: prior work has framed NHI governance primarily as a detection problem rather than a compliance measurement problem.
